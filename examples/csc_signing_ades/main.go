...
// This example targets ETSI PAdES (CAdES detached) and can optionally add RFC3161 timestamps.
package main
...
// SignPDFWithCSC signs a PDF using CSC remote signing (PAdES, optional timestamp).
func SignPDFWithCSC(ctx context.Context, pdfData []byte, config *CSCConfig, accessToken, sad string) ([]byte, error) {
	...
	if tsaURL != "" {
		timestamper := timestamps.NewHTTPTimestamper(tsaURL)
		user := os.Getenv("TSA_USER")
		pass := os.Getenv("TSA_PASS")
		if user != "" || pass != "" {
			timestamper.SetCredentials(user, pass)
		}
		opts.Timestamper = timestamper
		fmt.Printf("Using RFC3161 timestamper: %s\n", tsaURL)
	} else {
		fmt.Println("TSA_URL not set; creating a baseline AdES signature without timestamp")
	}

	// Sign the PDF
	fmt.Println("Signing PDF...")
	signedData, err := signers.SignPdfBytes(pdfData, metadata, cmsSigner, opts)
	if err != nil {
...
func getTestdataDir() string {
	...
	// Try from examples/csc_signing_ades directory
	if _, err := os.Stat("../../testdata"); err == nil {
		return "../../testdata"
	}
...
func main() {
	fmt.Println("CSC (Cloud Signature Consortium) PAdES AdES Signing Example")
	fmt.Println("===============================================================")
	fmt.Println()
	...
	if err != nil {
		...
		fmt.Println("  TSA_URL            - RFC3161 TSA endpoint for timestamps")
		fmt.Println("  TSA_USER           - TSA username (optional)")
		fmt.Println("  TSA_PASS           - TSA password (optional)")
		os.Exit(1)
	}
	...
	if len(os.Args) > 2 {
		...
		outputPath = strings.TrimSuffix(pdfPath, filepath.Ext(pdfPath)) + "_csc_ades_signed.pdf"
	}
	...
	fmt.Println("\nCSC PAdES AdES signing completed successfully!")
}
